...
\emph{PicoBlaze} is a 8-bit processor created by Xilinx Inc. for their Spartan and Virtex FPGA\footnote{Field-Programmable Gate Array} series as an embeddable circuit to implement sequential programming in the parallel FPGA architecture. This means the processor is not meant to be physically manufactured, yet it would be possible \cite{PicoBlazeWeb}.
    ...
    Historically, the name of the design was \texttt{KCPSM}, for Ken Chapman's Programmable State Machine and then Constant Coded Programmable State Machine. This term is now still used in the FPGA design - the \emph{VHDL}\footnote{\emph{VHSIC} (Very High Speed Integrated Circuit) Hardware Description Language} and Verilog components are still caled \texttt{kcpsmX} where \texttt{X} stands for the version of the processor \cite{DSPwFPGA}.
        ...
        The multi-byte instructions for arithmetics (\texttt{ADDCY} and \texttt{SUBCY}) were included since the very beginning.
    ...
    Several example source files were compiled and ran in a simulator to prove the resulting assembly corresponds to the input source code.
        ...
        It is also possible to test \texttt{ZERO} and \texttt{CARRY} flags of the processor.
            ...
            \begin{tabular}{ r | l }
                ...
                & register dump. The value can be contained be anywhere. \\
                ...
                & Contains a value for each of the registers register in the bank. \\
        ...
        \subsection{Test Cases}
            ...
            \begin{tabular}{ r | l }
                ...
                \texttt{arith.c} & Addition and subtraction \\
                \texttt{logic.c} & Binary operation on the variables \\
                \texttt{muldiv.c} & Multiplication and division \\
                \texttt{cond\_simple.c} & Simple condition evaluation \\
                \texttt{cond\_complex.c} & Complicated condition evaluation \\
                \texttt{func\_basic.c} & Basic function calling \\
                \texttt{func\_ptr.c} & Function pointer calling \\
                \texttt{func\_recurs.c} & Recursive function calling \\
                \texttt{struct.c} & Structure operations \\
            \end{tabular}
            \end{center}

    \section{Evaluation}

    \todo{dolpnit/prepsat podle toho jak to nakonec vyjde}
...
\hline
\hline\verb|basic|       &\checkmark             &\checkmark               &          \checkmark\\
\hline\verb|arith|       &\checkmark             &\checkmark               &$\times$(no support)\\
\hline\verb|logic|       &\checkmark             &\checkmark               &   $\times$(invalid)\\
\hline\verb|cond_simple| &\checkmark             &\checkmark               &          \checkmark\\
\hline\verb|cond_complex|&$\times$(invalid)      &\checkmark               &          \checkmark\\
\hline\verb|func_basic|  &\checkmark             &\checkmark               &          \checkmark\\
\hline\verb|func_ptr|    &\checkmark             &$\times$(no support)     &$\times$(no support)\\
\hline\verb|func_recurs| &\checkmark             &\checkmark               &$\times$(invalid)   \\
\hline\verb|struct|      &\checkmark             &\checkmark               &$\times$(no support)\\
